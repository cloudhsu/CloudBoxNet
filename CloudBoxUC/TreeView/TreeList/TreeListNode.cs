...
namespace CloudBox.Controller
{
	...
	public class NodeCollection : IEnumerable
	{
		...
		public Node slowGetNodeFromVisibleIndex(int index)
		{
			...
			CloudBox.Controller.Tracing.StartTrack(0);
			...
			while (iterator.MoveNext())
			{
				...
				if (index < 0)
				{
					CloudBox.Controller.Tracing.EndTrack(0, "slowGetNodeFromVisibleIndex({0})", startindex);
					return iterator.Current as Node;
				}
			}
			CloudBox.Controller.Tracing.EndTrack(0, "slowGetNodeFromVisibleIndex (null)");
			return null;
		}
